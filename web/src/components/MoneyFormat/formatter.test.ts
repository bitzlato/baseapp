...
describe('formatter', () => {
  ...
  test('should return the formatted amount', () => {
    ...
    expect(format(fromDecimal('123.123456', btcCurrency), { maxFractionDigits: 4 })).toEqual(
      '123.1234',
    );
    ...
    expect(
      format(fromDecimal('43.55', usdCurrency), { maxFractionDigits: 1, minFractionDigits: 0 }),
    ).toEqual('43.5');
  ...
  test('should return formatted by locale', () => {
    expect(formatByLocale('en', fromDecimal('10344000.35010000', btcCurrency))).toEqual(
      '10,344,000.3501',
    );
    expect(formatByLocale('ru', fromDecimal('10344000.35010000', btcCurrency))).toEqual(
      '10 344 000,3501',
    );
  });
  ...
  test('should NOT remove trailing zeros ', () => {
    expect(format(fromDecimal('0.00010000', btcCurrency), { removeTrailingZeros: false })).toEqual(
      '0.00010000',
    );
    expect(format(fromDecimal('123.0111', btcCurrency), { removeTrailingZeros: false })).toEqual(
      '123.01110000',
    );
  });
});
