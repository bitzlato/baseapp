...
export const getVerificationStep = (labels: Label[]) => {
  const lastVerifiedStep =
    labels.length &&
    labels
      .slice()
      .reverse()
      .find((label: Label) => label.value === 'verified' && label.scope === 'private');
  const lastVerifiedStepKey = lastVerifiedStep ? lastVerifiedStep.key : '';
  const lastVerifiedStepIndex = kycSteps().findIndex(
    (step: string) => step === lastVerifiedStepKey,
  );
