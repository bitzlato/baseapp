...
export const TradeTipsModal: FC = () => {
  ...
  const cryptocurrency = useMemo(() => trade.cryptocurrency.code, [trade.cryptocurrency.code]);

  const { data: p2pWallet } = useFetchP2PWallet(cryptocurrency);

  const balance = useMemo(() => p2pWallet?.balance, [p2pWallet]);

  const handleSelectTip = useCallback((key: keyof typeof tipValues) => setSelectedTip(key), []);
