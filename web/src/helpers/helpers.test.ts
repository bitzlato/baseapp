...
describe('Helpers', () => {
  ...
  it('Should return correct params', () => {
    expect(helpers.getHistorySagaParam({ page: 0, limit: 25, type: 'deposits' })).toBe(
      'page=1&limit=25',
    );
    expect(helpers.getHistorySagaParam({ page: 0, limit: 10, type: 'withdraws' })).toBe(
      'page=1&limit=10',
    );
    expect(helpers.getHistorySagaParam({ page: 0, limit: 5, type: 'trades' })).toBe(
      'page=1&limit=5',
    );
    expect(
      helpers.getHistorySagaParam({ page: 0, currency: 'btc', type: 'deposits', limit: 6 }),
    ).toBe('page=1&currency=btc&limit=6');
    expect(
      helpers.getHistorySagaParam({ page: 0, currency: 'btc', type: 'withdraws', limit: 25 }),
    ).toBe('page=1&currency=btc&limit=25');
    expect(
      helpers.getHistorySagaParam({ page: 0, limit: 25, type: 'trades', market: 'btcusd' }),
    ).toBe('page=1&limit=25&market=btcusd');
    expect(
      helpers.getHistorySagaParam({ page: 0, limit: 25, type: 'trades', time_from: '1557878400' }),
  ...
  it('Should return correct params', () => {
    ...
    expect(
      helpers.getUserAgent('Mozilla/4.0 (compatible; MSIE 5.0; Windows 95) Opera 6.01 [en]'),
    ).toBe('Opera 6 Windows 95');
  });
