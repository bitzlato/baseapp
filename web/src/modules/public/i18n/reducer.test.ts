...
describe('ChangeLanguage reducer', () => {
  it('should handle CHANGE_LANGUAGE', () => {
    ...
    expect(changeLanguageReducer(initialChangeLanguageState, actions.changeLanguage('ru'))).toEqual(
      expectedState,
    );
    expect(localStorage.getItem('lang_code')).toEqual('ru');
    expectedState = { lang: 'en' };
    expect(changeLanguageReducer(initialChangeLanguageState, actions.changeLanguage('en'))).toEqual(
      expectedState,
    );
