...
class IdentityComponent extends React.Component<Props, IdentityState> {
  ...
  public render() {
    ...
    const cityGroupClass = cr('pg-confirm__content-identity__forms__row__content', {
      ...
      'pg-confirm__content-identity__forms__row__content--wrong':
        city && !this.handleValidateInput('city', city),
    });
          ...
          <div className="pg-confirm__content-identity__forms__row input-group">
            ...
            <fieldset className={postcodeGroupClass}>
              <CustomInput
                ...
                placeholder={
                  postcodeFocused ? '' : this.translate('page.body.kyc.identity.postcode')
                }
  ...
  private scrollToElement = (displayedElem: number) => {
    const element: HTMLElement = document.getElementsByClassName(
      'pg-confirm__content-identity__forms__row',
    )[displayedElem] as HTMLElement;
  ...
  private handleCheckButtonDisabled = () => {
    const { city, dateOfBirth, firstName, lastName, postcode, residentialAddress, countryOfBirth } =
      this.state;
    ...
    const residentialAddressValid = this.handleValidateInput(
      'residentialAddress',
      residentialAddress,
    );
  ...
  private sendData = (event) => {
    ...
    const isIdentity =
      labels.length &&
      labels.find((w) => w.key === 'profile' && w.value === 'verified' && w.scope === 'private');
    const verifiedProfiles = user.profiles.length
      ? user.profiles.filter((i) => i.state === 'verified')
      : [];
    const lastVerifiedProfile =
      verifiedProfiles.length && verifiedProfiles[verifiedProfiles.length - 1];
