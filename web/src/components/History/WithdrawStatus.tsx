...
export const WithdrawStatus: React.FC<Props> = ({ item, currency }) => {
    ...
    switch (item.state) {
        ...
        case 'confirming':
            return (
                <ConfirmingStatus currency={currency} txid={item.blockchain_txid} confirmations={item.confirmations} />
            );
        case 'succeed':
            return (
        ...
        case 'errored':
            return item.public_message ? (
                ...
                <Status type="failed">{t(`page.body.history.withdraw.content.status.${item.state}`)}</Status>
            );
        case 'under_review':
            return <Status type="failed">{t('page.body.history.withdraw.content.status.under_review')}</Status>;
        default:
            return (
