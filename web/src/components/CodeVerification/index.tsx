...
const CodeVerification: FC<CodeVerificationProps> = (
  props: CodeVerificationProps,
): ReactElement => {
  const { code, codeLength, inputMode, isMobile, onSubmit, placeholder, showPaste2FA, type } =
    props;
  ...
  const getCodeBlocks = useCallback(() => {
    ...
    return codeItems.map((item, i) => {
      const cx = classnames('pg-code-verification__wrapper-item', {
        ...
        'pg-code-verification__wrapper-item-placeholder-border':
