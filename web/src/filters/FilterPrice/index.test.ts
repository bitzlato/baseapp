...
describe('Describe validatePriceStep filter', () => {
  it('should validate significant_digits filter', () => {
    ...
    expect(
      validatePriceStep(100, [{ type: 'significant_digits', digits: 5 }].map(buildFilterPrice)),
    ).toEqual({ valid: true, priceStep: 0 });
    expect(
      validatePriceStep(100.001, [{ type: 'significant_digits', digits: 5 }].map(buildFilterPrice)),
    ).toEqual({ valid: false, priceStep: 0.01 });
    expect(
      validatePriceStep(100.001, [{ type: 'significant_digits', digits: 5 }].map(buildFilterPrice)),
    ).toEqual({ valid: false, priceStep: 0.01 });
  });
