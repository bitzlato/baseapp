...
class ProfileVerificationComponent extends React.Component<Props> {
      ...
      case 'submitted':
        return (
          <OverlayTrigger
            ...
            overlay={
              <Tooltip title={`page.body.profile.verification.progress.tooltip.${step}.pending`} />
            }
          >
      ...
      default:
        return (
          <OverlayTrigger
            ...
            delay={{ show: 250, hide: 400 }}
            overlay={
              <Tooltip title={`page.body.profile.verification.progress.tooltip.${step}.default`} />
            }
          >
    ...
    return (
      <div className="pg-profile-page-verification__progress-bar">
        {kycSteps().map((step, index) =>
          React.cloneElement(this.renderProgressBarStep(step, index, labels), { key: index }),
        )}
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <OverlayTrigger
              ...
              overlay={
                <Tooltip
                  title={`page.body.profile.verification.${labelToCheck}.rejected.tooltip`}
                />
              }
            >
  ...
  public render() {
    ...
    return (
      <div className="pg-profile-page__box pg-profile-page-verification">
        ...
        {kycSteps().map((step: string, index: number) =>
          this.renderVerificationLabel(labels, step, index),
        )}
      </div>
    );
  }
  ...
  private handleCheckLabel = (labels: Label[], labelToCheck: string) => {
    const targetLabel =
      labels.length &&
      labels.find((label: Label) => label.key === labelToCheck && label.scope === 'private');
